...
USB_MOUNT_COMPAT_ALIASES="${USB_MOUNT_COMPAT_ALIASES:-true}"
...
scan_once() {
    ...
    while IFS=$'\t' read -r disk part uuid label fstype; do
        ...
        if [ "$role" = "VAULT" ] || [ "$role" = "SCRATCH" ]; then
            ...
            dir_name="$(named_mount_dir "$letter" "$label")"
            mount_point="$primary_root/$dir_name"
            raw_mount_point="$DRIVES_STATE_DIR/raw/${letter}-raw"

            local service_mount_point="$primary_root/$letter"
            ...
            status="$(mount_role_drive "$letter" "$part" "$uuid" "$label" "$fstype" "$service_mount_point" "${bind_targets[@]}")"
            case "$status" in
                mounted)
                    state="mounted"
                    ;;
    ...
    if [ "$detected" -eq 0 ]; then
        local fallback_role=""
        for fallback_role in VAULT SCRATCH; do
            local fallback_mount_role
            local fallback_uuid
            local fallback_label
            local fallback_letter
            local fallback_dir_name
            local fallback_source=""
            local fallback_state="unmounted"
            local fallback_error="role drive not detected"
            local fallback_mount_point=""
            local fallback_base=""
            local fallback_candidate=""

            fallback_mount_role="$(printf '%s' "$fallback_role" | tr '[:upper:]' '[:lower:]')"
            if [ "$fallback_role" = "VAULT" ]; then
                fallback_uuid="$USB_MOUNT_VAULT_UUID"
                fallback_label="$USB_MOUNT_VAULT_LABEL"
            else
                fallback_uuid="$USB_MOUNT_SCRATCH_UUID"
                fallback_label="$USB_MOUNT_SCRATCH_LABEL"
            fi

            fallback_letter="$(role_letter "$fallback_role")"
            fallback_dir_name="$(named_mount_dir "$fallback_letter" "$fallback_label")"
            fallback_mount_point="$primary_root/$fallback_dir_name"
            ...
            if [ -n "$fallback_source" ]; then
                ...
                fallback_targets+=("$primary_root/$fallback_letter")
                for fallback_base in "${mount_paths[@]}"; do
                    fallback_targets+=("$fallback_base/$fallback_letter")
                    fallback_targets+=("$fallback_base/$fallback_dir_name")
                done
                local fallback_target=""
                for fallback_target in "${fallback_targets[@]}"; do
                    ensure_bind_mount_path "$fallback_source" "$fallback_target" || true
                done
                fallback_state="mounted"
                fallback_error=""
                detected=$((detected + 1))
            fi

            drives_json+="$drives_sep{\"device\":\"-\",\"dirName\":\"$(json_escape "$fallback_dir_name")\",\"error\":\"$(json_escape "$fallback_error")\",\"filesystem\":\"unknown\",\"letter\":\"$(json_escape "$fallback_letter")\",\"mountPoint\":\"$(json_escape "$fallback_mount_point")\",\"mountRole\":\"$(json_escape "$fallback_mount_role")\",\"name\":\"$(json_escape "$fallback_label")\",\"rawMountPoint\":\"\",\"role\":\"$(json_escape "$fallback_role")\",\"state\":\"$(json_escape "$fallback_state")\",\"uuid\":\"$(json_escape "$fallback_uuid")\"}"
            drives_sep=","
        done
    fi
